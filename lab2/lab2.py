        ...
        if len(self.alunos) == 0:
            ...
            for i in range(len(self.alunos)):
                alunos_dados += "{} {} matrícula {}\n".format(
                    self.alunos[i].nome, self.alunos[i].DRE, self.alunos[i].matricula)
...
class Pessoa:
    ...
    def idade(self, data=date.today().strftime("%d/%m/%Y")):
        ...
        data_nasc = date(int(self.dataNascimento[6:]), int(
            self.dataNascimento[3:5]), int(self.dataNascimento[:2]))

        data_final = date(int(data[6:]), int(data[3:5]), int(data[:2]))

        if data_final.month < data_nasc.month:
            return (data_final.year - data_nasc.year) - 1
        elif data_final.month == data_nasc.month:
            if data_final.day < data_nasc.day:
                return data_final.year - data_nasc.year - 1
            else:
                return data_final.year - data_nasc.year
        else:
            return data_final.year - data_nasc.year


if __name__ == "__main__":
    leo = Pessoa('Leo', '13/09/2001', 'Elaine', 'Julio')
    print(leo)
    print(leo.dataNascimento)
    print(str(leo.idade()))
    print(str(leo.idade('22/10/2025')))
